...
HALF_TIME = int(FPS * 0.5)

def mouse_in_win() -> Vector2:
...
class Gui:
    ...
    def toast(self, text: str) -> None:
        self.toast_surf = FONT.render(text, False, WHITE)
        self.toast_timer = 2 * FPS
        self.toast_state = ToastState.UP
        self.toast_pos = Vector2(
            WIN_WIDTH // 2 - self.toast_surf.get_width() // 2,
            WIN_HEIGHT + self.toast_surf.get_height()
        )
    ...
    def get_super_pos(self) -> Vector2:
        return Vector2()
    ...
    def step(self):
        ...
        if self.toast_surf is not None:
            if self.toast_state == ToastState.UP:
                self.toast_pos[1] -= 2
                if self.toast_pos[1] < WIN_HEIGHT - self.toast_surf.get_height() - 5:
                ...
                self.toast_pos[1] += 2
                if self.toast_pos[1] > WIN_HEIGHT:
                    self.toast_state = ToastState.NONE
...
class GuiElement(ABC):
    def __init__(self, *args, **kwargs):
        self.pos = kwargs.get('pos', Vector2())
    ...
    def render_surf(self, text=None):
        ...
        if text:
            self.text = text
        self.surf = FONT.render(self.text, True, WHITE)
...
class ComboSwitch(GuiElement):
    def __init__(self, *args, **kwargs):
        ...
        self.items: List[str] = kwargs.get('items', ['value1', 'value2'])
    ...
    def set_items(self, strings, mapping=None):
        ...
        for string in strings:
            ...
            surf = FONT.render(string_to_render, True, WHITE)
    ...
    def draw(self, win: pygame.Surface) -> None:
        ...
        polygon_right = [Vector2(self.size[0] - arrow_size / 2, arrow_border), Vector2(self.size[0] - arrow_border, self.size[1] / 2), Vector2(self.size[0] - arrow_size / 2, self.size[1] - arrow_border)]
        polygon_left = [Vector2(arrow_size / 2, arrow_border), Vector2(arrow_border, self.size[1] / 2), Vector2(arrow_size / 2, self.size[1] - arrow_border)]
...
class ImageToggle(GuiElement):
    def __init__(self, *args, **kwargs):
        ...
        self.value = kwargs.get('value', False)
        image_path = kwargs.get('path', None)
        self.generate_event = kwargs.get('generate_event', False)
        self.border = 1
        if image_path is not None:
            raise NotImplementedError()
        surf = kwargs.get('surf', None)
...
class Input(GuiElement):
    def __init__(self, *args, **kwargs):
        ...
        self.text = kwargs.get('text', '')
        self.surf = FONT.render(self.text, True, WHITE)
        self.cursor_speed = FPS // 4
        self.show_cursor = False
        self.timer = 0
        self.cursor_surf = FONT.render("|", True, (255,255,255))
    ...
    def step(self):
        if self.mode == InputMode.EDIT:
            ...
            if self.value != self.old_input_text:
                ...
                self.surf = FONT.render(str(render_text), True, WHITE)
...
class LoadBar(GuiElement):
    ...
    def draw(self, win: pygame.Surface) -> None:
        ...
        if self.max_value == 0:
            return
        size = Vector2(self.size[0] * (self.value / self.max_value), self.size[1])

        # draw bar left to right direction
        if self.direction == 1:
            pygame.draw.rect(win, TEXT_ELEMENT_COLOR, (button_pos, self.size), 2)
            pygame.draw.rect(win, self.bar_color, (button_pos + Vector2(2,2), size - Vector2(4,4)))
...
def test_gui():
    ...
    menu = StackPanel(orientation=HORIZONTAL, pos=Vector2(0,0), size=Vector2(64 * 3, 64))
    menu.insert(ImageToggle(key='map_tool', surf=icons[0], generate_event=True))
    menu.insert(ImageToggle(key='grid_tool', surf=icons[1], generate_event=True))
    menu.insert(ImageToggle(key='token_tool', surf=icons[2], generate_event=True))

    menu2 = StackPanel(orientation=HORIZONTAL, pos=Vector2(0,80), size=Vector2(800, 25))
    menu2.insert(Button(key='1', text='button1'))
    menu2.insert(UpDown(key='2'))
    menu2.insert(Toggle(key='3', text='toggle me'))
    menu2.insert(ComboSwitch(key='4', items=['option1', 'option2', 'option3']))
    menu2.insert(Text(text='input:'))
    menu2.insert(Input(key='5'))
